<%@page import="cn.com.rpg.entity.newsRpg"%>
<%@page import="cn.com.rpg.serviceImp.newsRpgServiceImp"%>
<%@page import="cn.com.rpg.service.newsRpgService"%>
			...
			<div class="dataTable">
				<table class="table" id="table">
					...
					<%
					    ...
					    List<newsRpg> list = newsService.selectAll();
					    limitList<newsRpg> limite = new limitList<newsRpg>(list,8);
					    //获取总页数
						int pageToal = limite.getPageTotal();
						//获取每页显示的内容的集合
						List<newsRpg> newList = limite.limitListElement(pageIndex);
						for (newsRpg news : newList) {
					%>
					<tr>
						<td><%=news.getTitle()%></td>
						<td><%=news.getAuthor()%></td>
						<td><%=news.getCreateDate()%></td>						  				  
					    <td><a href="javascript:if(confirm('确认是否删除此数据？'))location='<%=request.getContextPath()%>/deleteByNewsId?id=<%=news.getId()%>'"><input type="button" value="删除"></a>
					    </td>
						<%
							}
						%>
	...
	<!-- 分页查询  -->	
		     <div class="pageNum">
				<ul class="pageInfo">
					...
					if(pageIndex>1){				      				  				
					%>
					<li class="firstPage pages fl"><a href="/RPG/pages/news2.jsp?pageNo=<%=1%>">首页</a></li>
					<li class="pageUp pages fl"><a href="/RPG/pages/news2.jsp?pageNo=<%=pageIndex-1%>">上一页</a></li>
					<%}else{%>
					<li class="firstPage pages fl"></li>
					<li class="pageUp pages fl"></li>
					<%}%>
					<%for(int i=1;i<=pageToal;i++){ %>
					<li class="pagesNo pages fl"><a href="/RPG/pages/news2.jsp?pageNo=<%=i%>"><%=i %></a></li>		
					<%} %>
					
					<%
					if(pageIndex<pageToal){
					%>
					<li class="pageDown pages fl"><a href="/RPG/pages/news2.jsp?pageNo=<%=pageIndex+1%>">下一页</a></li>
					<li class="lastPage pages fl"><a href="/RPG/pages/news2.jsp?pageNo=<%=pageToal%>">尾页</a></li>
					<%} %>
				</ul>
			</div>
		</div>
