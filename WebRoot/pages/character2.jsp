...
<script>
	...
	function cutStr(len){
		var obj=document.getElementById('table').getElementsByClassName('profile');
		...
		<div class="dataTable">
			<table class="table" id="table">
				...
				<%
	                ...
	                //调用分页类的方法				
				    characterRpgService character = new characterRpgServiceImp();
				    List<characterRpg> list = character.selectAll();
				    limitList<characterRpg> limite = new limitList<characterRpg>(list,8);
				    //获取总页数
					int pageToal = limite.getPageTotal();
					//获取每页显示的内容的集合
					List<characterRpg> newList = limite.limitListElement(pageIndex);
					for (characterRpg characterRpg : newList) {
				%>
				<tr>
					<td><%=characterRpg.getName()%></td>
					<td><%=characterRpg.getHeight()%></td>
					<td><%=characterRpg.getWeight()%></td>				
				    <td><%=characterRpg.getJob()%></td>
				    <td><%=characterRpg.getBloodType()%></td>
				    <td class="profile"><%=characterRpg.getProfile()%></td>		
				    <td><input type="button" value="修改"><br><br>
				    <a href="javascript:if(confirm('确认是否删除此数据？'))location='<%=request.getContextPath()%>/deleteBycharacterId?id=<%=characterRpg.getId()%>'"><input type="button" value="删除"></a>
				    </td>
					<%
						}
					%>
    	...
    	<div class="pageNum">
				<ul class="pageInfo">
					<li class="pageToal fl"><%=pageIndex %>/<%=pageToal %>页</li>
						...
						if(pageIndex>1){				      				  				
						%>
						<li class="firstPage pages fl"><a href="/RPG/pages/character2.jsp?pageNo=<%=1%>">首页</a></li>
						<li class="pageUp pages fl"><a href="/RPG/pages/character2.jsp?pageNo=<%=pageIndex-1%>">上一页</a></li>
						<%}else{%>
						<li class="firstPage pages fl"></li>
						<li class="pageUp pages fl"></li>
						<%}%>
						<%for(int i=1;i<=pageToal;i++){ %>
						<li class="pagesNo pages fl"><a href="/RPG/pages/character2.jsp?pageNo=<%=i%>"><%=i %></a></li>		
						<%} %>
						
						<%
						if(pageIndex<pageToal){
						%>
						<li class="pageDown pages fl"><a href="/RPG/pages/character2.jsp?pageNo=<%=pageIndex+1%>">下一页</a></li>
						<li class="lastPage pages fl"><a href="/RPG/pages/character2.jsp?pageNo=<%=pageToal%>">尾页</a></li>
						<%} %>
				</ul>
			</div>
		</div>
